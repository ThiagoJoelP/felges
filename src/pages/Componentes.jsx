...
function Componentes() {
  ...
  const emptyForm = { nombre: '', costoMateriales: '', costoManoObra: '', costoLuz: '', fechaVigencia: '' }
  ...
  const handleEditar = (c) => {
    setForm({ nombre: c.nombre, costoMateriales: c.costoMateriales || '', costoManoObra: c.costoManoObra || '', costoLuz: c.costoLuz || '', fechaVigencia: c.fechaVigencia || '' })
    setEditandoId(c.id)
    setShowForm(true)
  }
    ...
    <div>
      ...
      {showForm && (
        <div className="card form-card">
          ...
          <div className="form-grid">
            <div className="form-group"><label>Nombre</label><input type="text" placeholder="Ej: PVC Azul" value={form.nombre} onChange={e => setForm({...form, nombre: e.target.value})} /></div>
            <div className="form-group"><label>Costo de materiales ($)</label><input type="number" placeholder="0.00" value={form.costoMateriales} onChange={e => setForm({...form, costoMateriales: e.target.value})} /></div>
            <div className="form-group"><label>Costo de mano de obra ($)</label><input type="number" placeholder="0.00" value={form.costoManoObra} onChange={e => setForm({...form, costoManoObra: e.target.value})} /></div>
            <div className="form-group"><label>Costo de luz ($)</label><input type="number" placeholder="0.00" value={form.costoLuz} onChange={e => setForm({...form, costoLuz: e.target.value})} /></div>
            <div className="form-group"><label>Fecha de vigencia</label><input type="date" value={form.fechaVigencia || today} onChange={e => setForm({...form, fechaVigencia: e.target.value})} /></div>
          </div>
      ...
      <div className="card" style={{marginTop: 20}}>
        ...
        <table className="data-table" style={{marginTop: 16}}>
          <thead><tr><th>Nombre</th><th>Costo Materiales</th><th>Costo M.O.</th><th>Costo Luz</th><th>Costo Total</th><th>Vigencia</th><th>Acciones</th></tr></thead>
          <tbody>
            {componentes.map(c => {
